...
+ (id)rsa {
    static RSAEncryptor *_shared = nil;
...
- (id)init {
    if (self = [super init]) {
        [self loadPublicKeyFromFile:[[NSBundle mainBundle] pathForResource:@"public_key" ofType:@"der"]];
        [self loadPrivateKeyFromFile:[[NSBundle mainBundle] pathForResource:@"private_key" ofType:@"p12"] password:@"liulonghuang"];
    }
